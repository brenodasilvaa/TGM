...
public class EsferaRepository : IEsferaRepository
{
    ...
    public async Task<List<ParityInfoModel>?> GetPartnersParities(CancellationToken cancellation)
    {
        ...
        var parities = JsonSerializer.Deserialize<Parities>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        return parities?.Items;
    }
}
